<template>
    <app-layout title="Admin Reviews">
        <template #header>
            <Breadcrumbs name="AdminReviews" />
        </template>
                    ...
                    <tr v-for="row in data.data" :key="row.id">
                        <td>
                            <div class="flex items-center">
                                ...
                                <Link :href="route('admin.reviews.show', [row.id])">{{ row.title }}</Link>
                            </div>
                        </td>
                        <td><Link :href="route('admin.things.edit', [row.id])">{{ row.thing.name }}</Link></td>
                        <td><Link :href="route('users.show', [row.id])">{{ row.user.first_name }} {{ row.user.last_name }}</Link></td>
                        <td>{{ row.created_at }}</td>
                        <td><Link :href="route('admin.reviews.edit', [row.id])">Edit</Link></td>
                    </tr>
        ...
        watch: {
            form: {
                ...
                handler: throttle(function () {
                    this.$inertia.get('/admin/reviews', pickBy(this.form), { preserveState: true })
                }, 150),
            },
        },
        methods: {
